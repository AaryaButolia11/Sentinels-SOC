...
# from models.enrichment import enrich_alert
...
from models.anomaly_model import AnomalyScorer
from models.classifier import AttackClassifier
from models.explainability import explain_classifier, explain_anomaly, fit_population_stats, build_shap_explainer
...
class Pipeline:
    def __init__(self, anomaly_threshold: float = 0.6):
        ...
        self._population = {"n_events_total": 0, "hour_histogram": {}, "resource_histogram": {}}
        self._feature_means, self._feature_stds = {}, {}
        # Built ONCE in fit(), reused for every event in process(). Rebuilding
        # a SHAP TreeExplainer per-event (the old behavior) walks the entire
        # tree ensemble every single call -- on hundreds of pre-scored events
        # at startup, that's the difference between finishing in seconds vs.
        # never finishing within a reasonable time.
    ...
    def fit(self, feature_rows: list[dict], labels: list[str], store: ProfileStore):
        ...
        self.classifier = AttackClassifier(use_smote=True).fit(feature_rows, labels)
        self._feature_means, self._feature_stds = fit_population_stats(feature_rows, FEATURE_NAMES)
        # Build the SHAP explainer once, right after the classifier is fit.
        try:
            self._shap_explainer = build_shap_explainer(self.classifier.model)
        except Exception:
            # If SHAP construction fails for any reason (e.g. unsupported
            # model type), fall back to per-call construction inside
            # explain_classifier rather than crashing training entirely.
            self._shap_explainer = None
        return self
        ...
        if risk_score >= self.anomaly_threshold:
            ...
            try:
                explanation = explain_classifier(
                    self.classifier.model, feats, FEATURE_NAMES,
